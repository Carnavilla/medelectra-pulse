        ...
        {isLoading ? (
          ...
          <div className="mt-10 grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {data?.map((c) => (
              <article
                key={c.id}
                className="rounded-xl border border-border bg-card p-6 flex flex-col teal-glow-hover"
              >
                <div className="text-2xl font-bold text-primary">{naira(c.price)}</div>
                <h3 className="mt-3 text-xl font-semibold leading-tight">{c.title}</h3>
